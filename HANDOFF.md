...
- Current build: **b166**. Recent: b166 fixes the chip freeze/jump staircase on Windows (Bug D, the P0 both Claude-workflow and Codex ranked highest) — the far-seek (gap>=2/release) restarted the page render for every page the finger pointed at, and renderSeq cancels the previous one, so the page never mounted while the number ran on. It is now SINGLE-FLIGHT (one load in the air; chase seeks the latest once it mounts; release supersedes a stale load once). Windows benefit confirmable on the tablet. Next: E finger scroll.
