...
- Current build: **b193**. Recent: b167 fixes finger-scroll (not chip) freeze/jump at a page join on Windows (Bug E) — during a remount the page no longer freezes under the finger (its travel is banked and applied after the swap re-anchors), and letting go mid-swap no longer starts a glide that fights the re-anchor (it hands off to the damped glideCarry). Both device-confirmable. This closes the A-H batch: A toolbar shift, B pen-down delay, C false notice, D chip staircase, E finger scroll, F immerse arrows, G cross-page ink shift, plus the build-not-updating SW fix.
...
- **b193** `PENDING` - **chunks you can see and make.** b192 built the store; this is the part on screen. The rule the whole build is arranged around: **a chunk arriving must add headings and move nothing.** Regrouping a list somebody is looking at moves pages under their finger, so the list asks a different question from the one the data layer answers - `chunkRuns` walks the pages in the order they are already in and starts a new heading wherever the chunk changes. A chunk whose pages are not together therefore shows up twice, which is honest: the sitting really was interrupted, and quietly gathering the pages back together would be a reshuffle. A notebook that has never made a chunk is painted exactly as before - no heading, no extra element, the same flat list. **A heading folds its own pages** rather than opening anything, and folded is never written down: it is about this device and this moment, and a chunk folded on the tablet must not arrive folded on the laptop. **Starting one**: a Chunk button next to Insert, born with the page you are on - there is no empty chunk waiting to be filled, because a grouping with nothing in it is only something to tidy up later. A working page or a short note cannot start one; a sheet is not in the reading order. **Keeping one**: a page written next joins the same sitting, or a chunk could never be more than the one page it was made from. **Undoing one**: long-press the heading and clear the name - the chunk goes and its pages stay exactly where they are. **Verified in-browser**: three pages flat with no headings; Chunk made "Tuesday session" over the open page with "Earlier pages" below it; a new page joined it and the count went 1 to 2; folding, renaming to "Week 2", and clearing the name left all four pages present and ungrouped. - *Claude Code (Opus 5)*
